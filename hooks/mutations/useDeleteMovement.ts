...
export function useDeleteMovement() {
  ...
  return useMutation({
    ...
    onMutate: async (deletedMovement) => {
      ...
      const transactionsToDelete = (previousTransactions as any[] || []).filter(
        (t: any) => t.movementId === deletedMovement.movementId
      );
        ...
        queryClient.setQueryData(
          ...
          (old: any[] = []) =>
            old.map((account) => {
              ...
              const balanceDelta = affectingTransactions.reduce((sum: number, t: any) => {
