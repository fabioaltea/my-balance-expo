      ...
      if (updatedMovement.transactions) {
        ...
        const newTransactions = updatedMovement.transactions.map((t, index) => {
          ...
          return {
            ...
            description: updatedMovement.description || oldTransactions[0]?.description || '',
            amount: Math.abs(amount),
            type: t.type === 'in' ? ('income' as const) : ('expense' as const),
            account: t.account,
            category: updatedMovement.category || oldTransactions[0]?.category || '',
            location: updatedMovement.location !== undefined ? updatedMovement.location : oldTransactions[0]?.location || '',
            recurrenceId: updatedMovement.recurrenceId !== undefined ? updatedMovement.recurrenceId : oldTransactions[0]?.recurrenceId,
            recurrencePattern: updatedMovement.recurrencePattern !== undefined ? updatedMovement.recurrencePattern : oldTransactions[0]?.recurrencePattern,
            status: updatedMovement.status !== undefined ? updatedMovement.status : oldTransactions[0]?.status || 'Confirmed',
          };
        });
          ...
          (old: any[] = []) =>
            old.map((transaction) => {
              if (transaction.movementId === updatedMovement.movementId) {
                return {
                  ...transaction,
                  ...(updatedMovement.description !== undefined && { description: updatedMovement.description }),
                  ...(updatedMovement.category !== undefined && { category: updatedMovement.category }),
                  ...(updatedMovement.date !== undefined && { date: updatedMovement.date }),
                  ...(updatedMovement.location !== undefined && { location: updatedMovement.location }),
                  ...(updatedMovement.status !== undefined && { status: updatedMovement.status }),
                  ...(updatedMovement.recurrenceId !== undefined && { recurrenceId: updatedMovement.recurrenceId }),
                  ...(updatedMovement.recurrencePattern !== undefined && { recurrencePattern: updatedMovement.recurrencePattern }),
                };
              }
              return transaction;
            })
        ...
        queryClient.setQueryData(
          ...
          (old: any[] = []) =>
            old.map((account) => {
              ...
              balanceDelta -= oldAffectingTransactions.reduce((sum: number, t: any) => {
                const amount = t.amount;
                return sum + (t.type === 'income' ? amount : -amount);
              }, 0);

              // Apply the effect of new transactions for this account
              const newAffectingTransactions = updatedMovement.transactions!.filter(
