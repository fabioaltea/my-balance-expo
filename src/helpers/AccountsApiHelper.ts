...
export class AccountsApiHelper {
    ...
    return {
      ...
      balance:
        typeof raw.balance === 'number'
          ? raw.balance
          : parseFloat(raw.balance?.replace?.(/[€\s]/g, '')?.replace?.(',', '.') || '0'),
  ...
  static async getAccounts(spreadsheetId: string): Promise<any[]> {
    try {
      ...
      const response = await HttpHelper.get(`/accounts?spreadsheet_id=${spreadsheetId}`);

      if (response.success) {
        console.log('✅ Accounts loaded successfully:', response.data?.length || 0);
  ...
  static async createAccount(spreadsheetId: string, accountData: any) {
    try {
      ...
      const response = await HttpHelper.post(`/accounts?spreadsheet_id=${spreadsheetId}`, {
        ...accountData,
      });
  ...
  static async updateAccount(spreadsheetId: string, accountId: string, accountData: any) {
    try {
