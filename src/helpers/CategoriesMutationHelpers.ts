...
export class CategoriesMutationHelpers {
  static async optimisticUpdateCategory(
    ...
    const previousCategories = queryClient.getQueryData<Category[]>(QUERY_KEYS.categories.all);

    queryClient.setQueryData<Category[]>(QUERY_KEYS.categories.all, (old = []) =>
      old.map((cat) =>
  ...
  static rollbackCategories(queryClient: QueryClient, context: CategorySnapshot | undefined): void {
    if (context?.previousCategories) {
      queryClient.setQueryData(QUERY_KEYS.categories.all, context.previousCategories);
    }
  }

  static invalidateCategoryCaches(queryClient: QueryClient, nameChanged: boolean): void {
