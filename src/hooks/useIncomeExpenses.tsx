...
export const useIncomeExpenses = ({
  ...
  const dateRange = useMemo(() => {
    ...
    const oldestDate = new Date(
      currentYear,
      currentMonth - monthsToShow + 1 - monthOffset,
      1,
    );
  ...
  const { data: aggregations, isSuccess } = useSpreadsheetQuery<
    MonthlyAggregation[]
  >({
    ...
    fetchFn: (id) =>
      AggregationsApiHelper.getMonthlyAggregations(
        id,
        dateRange.fromDate,
        dateRange.toDate,
      ),
  ...
  const useBackendData =
    useAggregations && isSuccess && aggregations && aggregations.length > 0;
