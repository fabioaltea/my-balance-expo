...
const SettingsItem: React.FC<SettingsItemProps> = ({
  ...
  >
    <View style={styles.settingsItemLeft}>
      ...
      <ThemedText
        style={[styles.settingsItemText, { color: isDestructive ? destructiveColor : textColor }]}
      >
        {title}
      </ThemedText>
    </View>
    {showChevron && <IconSymbol name="chevron-right" size={16} color={secondaryTextColor} />}
  </TouchableOpacity>
);
...
const SettingsSection: React.FC<SettingsSectionProps> = ({
  ...
  <View style={styles.section}>
    {title && (
      <ThemedText style={[styles.sectionTitle, { color: secondaryTextColor }]}>{title}</ThemedText>
    )}
    <View style={[styles.sectionContent, { backgroundColor: 'transparent' }]}>{children}</View>
  </View>
);
...
const SettingsView: React.FC<SettingsViewProps> = ({ user, logout }) => {
  ...
  const secondaryTextColor = useThemeColor({ light: '#8e8e93', dark: '#8e8e93' }, 'tabIconDefault');
  const separatorColor = useThemeColor({ light: '#c6c6c8', dark: '#38383a' }, 'tabIconDefault');
  ...
  const handleGenerateShortcut = async () => {
    try {
      ...
      if (ICLOUD_SHORTCUT_URL) {
        ...
        Alert.alert('Key Copied', 'Your shortcut key has been copied to clipboard.', [
          { text: 'OK' },
        ]);
      }
    } catch (error: any) {
      console.error('Failed to generate shortcut:', error);
      Alert.alert('Error', error.message || 'Failed to generate shortcut key', [{ text: 'OK' }]);
    } finally {
  ...
  const handleToggleNotifications = async (enabled: boolean) => {
    ...
    try {
      if (enabled) {
        const token = await NotificationsHelpers.registerForPushNotificationsAsync();
        if (token) {
          setNotificationsEnabled(true);
          console.log('Push notifications enabled with token:', token);
        }
      } else {
        const removed = await NotificationsHelpers.unregisterPushNotificationsAsync();
        if (removed) {
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      {/* Notifications */}
      <SettingsSection title="NOTIFICATIONS" secondaryTextColor={secondaryTextColor}>
        <InputGroup>
          <View
            style={[
              styles.settingsItem,
          ...
          >
            <View style={styles.settingsItemLeft}>
              ...
              <ThemedText style={[styles.settingsItemText, { color: textColor }]}>
                Push Notifications
              </ThemedText>
            </View>
      ...
      {/* Shortcuts */}
      <SettingsSection title="SHORTCUTS" secondaryTextColor={secondaryTextColor}>
        <InputGroup>
          <TouchableOpacity
            style={[
              styles.settingsItem,
